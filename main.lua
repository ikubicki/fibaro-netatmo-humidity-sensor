...
function QuickApp:onInit()
    ...
    self:trace('')
    self:trace('Netatmo humidity sensor')
    self:trace('User:', self.config:getUsername())
    self:updateProperty('manufacturer', 'Netatmo')
    self:updateProperty('manufacturer', 'Humidity sensor')
    self:run()
    self:updateView("label2", "text", string.format(self.i18n:get('humidity'), 0))
    self.prompted = false
end
...
function QuickApp:pullNetatmoData()
    ...
    local callback = function(response)
        ...
        if sensor ~= nil then
            ...
            self:trace('[' .. sensor['_id'] .. '] Humidity sensor information updated')
            self:updateView("label1", "text", string.format(self.i18n:get('last-update'), os.date('%Y-%m-%d %H:%M:%S')))
            self:updateView("label2", "text", string.format(self.i18n:get('humidity'), sensor.dashboard_data.Humidity))
            ...
            self:updateProperty('value', sensor.dashboard_data.Humidity)
